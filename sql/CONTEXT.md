...
| [escala/21_transcricoes_ligacao.sql](escala/21_transcricoes_ligacao.sql) | Tabela `transcricoes_ligacao` (sem FK p/ `ligacoes`, `call_id` UNIQUE alvo do upsert, `analise_ia` jsonb) — linha estruturada/queryável de cada ligação transcrita (transcrição, análise-IA, metadados, ponteiros de gravação), escrita RETRY-BACKED no processador (quick 260822-ubk). |
